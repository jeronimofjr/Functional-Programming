{-
Escreva uma função todosSufixos tal que (todosSufixos xs) devolve uma lista formada com todos 
os sufixos da lista xs. Um sufixo de uma lista xs é qualquer sequência final de elementos da 
lista xs. Por exemplo,
...
todosSufixos xs = [drop i xs | i <- [(length xs),(length xs-1)..0]]
